...
const COURSE_NAMES: Record<string, string> = {
  quranic: "Learn Quranic Arabic",
  recitation: "Recitation & Tajweed",
  memorization: "Hifz & Memorisation",
};
...
export default function OnboardingPage() {
  ...
  const goNext = () => {
    if (step === 0) {
      // Initialise student array from step 1 selection
      const count = whoFor === "myself" ? 1 : childCount;
      const arr = Array.from({ length: count }, () => emptyStudent());
      setStudents(arr);
    }
  ...
  const monthlyPrice = Math.round(basePrice * (course.daysPerWeek / 2) * (course.minutesPerClass / 30));
    ...
    >
      ...
      >
        ...
        {done ? (
          ...
          <Step3Course data={course} onChange={setCourse} onNext={goNext} onBack={goBack} />
        ) : (
          <Step4Schedule
            data={schedule}
